...
if(isset($_POST['cat_id'])){
   $cat= mysqli_real_escape_string($connection,$_POST['cat_id']);
...
FROM
    ...
    INNER JOIN `db_vsm`.`tbl_sub_category` 
        ON (`tbl_category`.`cat_id` = `tbl_sub_category`.`cat_id`) where `tbl_sub_category`.`cat_id`= '{$cat}'") or die(mysqli_error($connection));
...
FROM
    `db_vsm`.`tbl_category`
    INNER JOIN `db_vsm`.`tbl_sub_category` 
        ON (`tbl_category`.`cat_id` = `tbl_sub_category`.`cat_id`) where `tbl_sub_category`.`is_delete`='0' ") or die(mysqli_error($connection));
