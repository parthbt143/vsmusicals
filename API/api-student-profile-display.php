...
FROM
    ...
    INNER JOIN `db_vsm`.`tbl_student` 
        ON (`tbl_area`.`area_id` = `tbl_student`.`area_id`) where `tbl_student`.`stud_email`='{$email}' ") or die(mysqli_error($connection));
...
FROM
    `db_vsm`.`tbl_area`
    INNER JOIN `db_vsm`.`tbl_student` 
        ON (`tbl_area`.`area_id` = `tbl_student`.`area_id`)  where `tbl_student`.`is_delete`=0") or die(mysqli_error($connection));
